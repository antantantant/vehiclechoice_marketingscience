...
public class RealChromosome extends AbstractListChromosome<Double> {
		...
		public RealChromosome(List<Double> representation, double fitness) {
			super(representation);
			this.representation = representation;
			this.fitness = fitness;
		}
		
		public static List<Double> randomRealRepresentation(int length) {
			// random Real list
			List<Double> rList= new ArrayList<Double> (length);
			for (int j=0; j<length; j++) {
				rList.add(GeneticAlgorithm.getRandomGenerator().nextDouble());
			}
			return rList;
		}
		...
		protected boolean isSame(Chromosome another) {
			if (! (another instanceof RealChromosome))
				return false;
			RealChromosome anotherRc = (RealChromosome) another;
			// size check
			if (getLength() != anotherRc.getLength())
				return false;

			for (int i=0; i< getRepresentation().size(); i++) {
				if (!(getRepresentation().get(i).equals(anotherRc.getRepresentation().get(i))))
					return false;
			}
			return true;
		}
		...
		@Override
		protected void checkValidity(List<Double> arg0)
		...
		public AbstractListChromosome<Double> newFixedLengthChromosome(
				...
				double[] std, int[][] INDa, int[][] INDb, int[][] INDc, double obj, double[][] allX) {
			RealChromosome c = new RealChromosome(
					representation, calculateFitness(list2prim(representation),
							sv, w, lambda, mean, std, INDa, INDb, INDc, obj, allX));
			return c;
		}
		...
		private double calculateFitness(double[] x, double[][] sv, double[] w, double lambda, 
				double[] mean, double[] std, int[][] INDa, int[][] INDb, int[][] INDc, double obj, double[][] allX) {
			if (obj==0){
				return distance(x, allX);
			}
			else if(obj==-1){//find the optimal design in post analysis
				return decisionf(x, sv, w, lambda, mean, std, INDa, INDb, INDc);
			}
			else{
				return merit(0.99,0.01,
						Math.exp(-Math.abs(obj-decisionf(x, sv, w, lambda, mean, std, INDa, INDb, INDc))),distance(x,allX));
			}
		}
		...
		private double decisionf(double[] x, double[][] X, double[] w, double lambda, 
...
//			double[] feature = x.clone();
			...
			for (int i = 0; i<nfeature; i++){
				feature[i]= (feature[i]-mean[i])/(std[i]+1e-16);
			}
...
//				badfeature = X[INDa[i][1]].clone();
				...
				for (int j = 0; j<nfeature; j++){
					goodfeature[j]= (goodfeature[j]-mean[j])/(std[j]+1e-16);
					badfeature[j]= (badfeature[j]-mean[j])/(std[j]+1e-16);
				}
...
//				badfeature = X[INDc[i][1]].clone();
		...
		private double kernel(double[] v1, double[] v2, double lambda) {
			if (lambda>0){
				int l = v1.length;
				double k = 0;
				for (int i = 0;i<l;i++){
		...
		private double[] calOneFeature(double[] x){
			...
			double[][] controlPoints = getControlPoints(x);
...
//		private double defdotsvm(double[] x, svm_node[] ynode){
//			double f = 0;
//			for (int i = 0; i<x.length; i++){
//				f+= (x[i]-ynode[i].value)*(x[i]-ynode[i].value);
//			}
//			return f;
//		}
		
//		private double defdot(double[] x, double[] y){
//			double f = 0;
//			for (int i = 0; i<x.length; i++){
//				f+= (x[i]-y[i])*(x[i]-y[i]);
//			}
...
//		}
		...
		private double[][] getControlPoints(double[] Input) {
		    int order = 2;
		    ...
		    double hood_n = (hood_start_pt[0] - hood_end_pt[0])*0.1/ hood_vec[0] +  (hood_start_pt[0] - hood_end_pt[0]) * 0.9 /  hood_vec[0] * Input[12];
		    ...
		    double[] tempv9 = {fwheel_x - well_scale*r_wheel*0.5, 0, r_wheel*well_scale*0.5*Math.sqrt(3) + fwheel_z};
		...
		private double[][] init_berstein_polys(int order, int step, double[] CC) {
		    double[][] BC = new double[order+1][step+1];
		    double t_step = 1.0/step;
		    double t;
		    double tC;
