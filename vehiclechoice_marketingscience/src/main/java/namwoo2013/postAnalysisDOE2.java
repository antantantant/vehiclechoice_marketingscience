...
public class postAnalysisDOE2 {
	...
	// in case styling modeling is skipped during data collection
	public static void trainDOE(String data) throws JSONException{
		JSONObject js = new JSONObject(data);
		...
		if (id+n<=totalUserNum){
			...
			for (TestDOE t:q){
				if (count>=id&&count<Math.min(id+n,totalUserNum)){
					...
					// if no styling model is trained during data collection, 
					// it has to be trained when downloading data if required
//					trainDOE(data);
					// otherwise use the following line to directly read the model
					...
					for(int i=0;i<s_X.length;i++){
						u_X[i][0] = s_decisionf(s_X[i], count-id);
					}
	...
	private static double s_decisionf(double[] x, int id){
		...
		if(INDb!=null){
			for (int i = 0; i<INDb.length;i++){
				...
				f+= w[INDalength+i]*(kernel(goodfeature,feature)-kernel(badfeature,feature));
			}
			INDblength = INDb.length;
		}
		else{
			INDblength = 0;
		}
		if(INDc!=null){
			for (int i = 0; i<INDc.length;i++){
				...
				f+= w[INDalength+INDblength+i]*(kernel(goodfeature,feature)-kernel(badfeature,feature));
			}
		}
		return f;
	}
	
	private static double decisionf(double[] x){
		double f = 0;
		double[] feature = calOneFeature(x);
		double nfeature = feature.length;
		int INDalength, INDblength;
