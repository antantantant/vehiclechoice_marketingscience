...
test.prototype.initiate = function(){
	...
	var data = "n="+(2*ntest)+"&action=getDOE";
	...
	request.onreadystatechange = function(){
		if ((request.readyState == 4) && (request.status == 200)) {
			...
			$("div#attribute2frame1").html("<a>"+t.MPGSet[Math.round(t.A[0][2]*(t.MPGLevel-1))]+"</a><a style='font-size:100%'>MPG<br>(City/Hwy)");
			$("div#attribute1frame1").html("<a>"+t.priceSet[Math.round(t.A[0][1]*(t.priceLevel-1))]+"</a><a style='font-size:100%'>,000 USD (MSRP)");
			$("div#attribute2frame2").html("<a>"+t.MPGSet[Math.round(t.A[1][2]*(t.MPGLevel-1))]+"</a><a style='font-size:100%'>MPG<br>(City/Hwy)");
			$("div#attribute1frame2").html("<a>"+t.priceSet[Math.round(t.A[1][1]*(t.priceLevel-1))]+"</a><a style='font-size:100%'>,000 USD (MSRP)");
			$('#leftmost').hide();
			$('#rightmost').hide();	
		}
	};
...
//		$('#question').html(question2);
//		$("div#attribute2frame1").html("<a>"+(t.A[t.iter*2][2]*t.MPGRange+t.baseMPG)+"</a><a style='font-size:100%'>MPG");
//		$("div#attribute1frame1").html("<a>"+(t.A[t.iter*2][1]*t.priceRange+t.basePrice)+"</a><a style='font-size:100%'>USD");
//		$("div#attribute2frame2").html("<a>"+(t.A[t.iter*2+1][2]*t.MPGRange+t.baseMPG)+"</a><a style='font-size:100%'>MPG");
//		$("div#attribute1frame2").html("<a>"+(t.A[t.iter*2+1][1]*t.priceRange+t.basePrice)+"</a><a style='font-size:100%'>USD");
...
test.prototype.attributeChosenAction = function(){
	wspinner.show();
...
//	if (!cb){ // normal order: first style then attributes
	    if (t.iter<t.ntest-1){
			...
			t.currentX[1] = t.X[2*t.iter+1].slice(0);
		    this.show();
			$("div#status p").html("<p>There are "+(t.ntest-t.iter+t.nvalidate*2)+" comparisons remaining.</p>");
			$('#question').html(question2);
...
test.prototype.store = function(){
	...
	data = "data="+data+"&action=storeDOE";
	var request = getRequestObject();
	request.onreadystatechange = function(){
		getMTurkCodePost();
	};
