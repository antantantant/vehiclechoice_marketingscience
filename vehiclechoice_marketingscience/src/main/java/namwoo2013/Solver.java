...
// the following code solves the dual problem of ranking svm.
...
// you can certainly use an existing package
// however, please check if the package deals with ordinal labels in pairwise comparisons
// (e.g., slightly better, much better)
...
public class Solver {
	...
	{	
		...
		double[] pid;
		double Gnorm; double dinorm; double sum_alpha; int Gpos;
		int[] set;
			...
			while (con&&count<1e1){
				...
				while (con) {
					...
					if (j==-1){
						...
						set = selectB();
						i = set[0]; j = set[1];
						if (((Gnorm<0.05||dinorm<eps) && Gpos == l) || (iter>=1e3 && sum_alpha>0.0)){
							con = false;
